...
@Component
@Slf4j
public class AskRankHandler implements JobApplicationFlowHandler{
    ...
    public SendMessage handle(Long chatId, String message, ConcurrentHashMap<Long, String> userState,
        ...
        if(state.equals("ASK_RANK")){
            userData.get(chatId).setMilitaryRank(message);
            userState.put(chatId, "ASK_COMBAT_PARTICIPANT");
            return new SendMessage(chatId, "Чи є у вас посвідчення учасника бойових дій?")
            ...
            log.info("Error occured due to the AskRankHandler processing");
