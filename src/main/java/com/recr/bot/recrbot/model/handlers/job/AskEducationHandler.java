...
@Component
@Slf4j
public class AskEducationHandler implements JobApplicationFlowHandler {
    ...
    public SendMessage handle(Long chatId, String message, ConcurrentHashMap<Long, String> userState,
            ConcurrentHashMap<Long, CandidatEntity> userData, String state, KeyboardController keyboardController) {
                if(state.equals("ASK_EDUCATION")){
                    userData.get(chatId).setRelationshipStatus(message);
                    ...
                    log.info("Error ocured due to AskEducatuinHandler processing");
